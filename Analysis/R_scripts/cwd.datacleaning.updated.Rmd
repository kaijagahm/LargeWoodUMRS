...
sites_ur <- sites_ur %>% dplyr::select(-toexclude) # drop columns
names(sites_ur)
```
...
colstomodify <- sites_ur %>% dplyr::select(-factors) %>% dplyr::select(-barcode) %>% names()
...
sites_ur <- sites_ur %>% 
  dplyr::mutate(stratum_collapsed = 
                  case_when(stratum %in% c("BWC-O", "BWC-S") ~ "BWC",
                            ...
                            stratum %in% c("MCB-U", "MCB-W") ~ "MCB",
                            stratum  == "SCB" ~ "SCB"))
sites_ur <- sites_ur %>% 
  mutate(stratum = stratum_collapsed) %>%
  filter(!is.na(stratum)) %>%
  dplyr::select(-stratum_collapsed)

table(sites_ur$stratum, exclude = NULL)
```
...
table(sites_lr$stratum, exclude = NULL)
```
