...
library(tidyverse)
sites_upperlower %>% ggplot(aes(x = utm_e, y = utm_n, col = pool))+
  geom_point()

sites_upperlower %>% filter(pool == "OR2") %>% ggplot(aes(x = utm_e)) + geom_histogram()

sites_upperlower %>% filter(pool == "OR2", utm_e < 400000)
```
