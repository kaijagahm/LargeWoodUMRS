...
fish_data_EF_tojoin <- fish_data_EF %>% select(-c(site, stratum, lcode, sdate, utm_e, utm_n, snag))
...
sites_aa_5m <- sites_aa_5m %>% select(-c(joiner, TARGET_FID, pool)) # remove pool from this one because it's a factor here, not a number like it is in sites_ur.
...
## Remove unnecessary columns from fishdata
```{r}
# Which do we want from fishdata?
tokeep_fishdata <- c("year", "sitetype", "gear", "wingdyke", "riprap", "trib", "barcode", "summary")
all_fishdata <- names(fish_data_EF)
todrop_fishdata <- all[all %notin% tokeep] # to drop from fish data

todrop_sites_ur <- names(sites_ur)[names(sites_ur) %in% todrop_fishdata] # to drop from sites_ur

sites_ur <- sites_ur %>% select(-todrop_sites_ur) # drop columns
```
...
# Load data (Lower River-- 26, La Grange, Open River)
```{r}
# Load files
...
ltrm_fish_data <- ltrm_fish_data_new

sites_p26ilror <- read.csv("../data/inputs/DataSets_8_17/AttributeTables/sites_p26ilror.txt") # reprojected data
...
### Add `year` and `row` column to ltrm_fish_data
```{r}
ltrm_fish_data <- ltrm_fish_data %>% mutate(year = str_extract(sdate, pattern = "[[:digit:]]{4}$"),
                                            row = 1:nrow(.))
```

### Join barcodes to reprojected data
```{r}
sites_p26ilror <- sites_p26ilror %>% rename(row = RowID_) # rename row column
barcodes_tojoin <- ltrm_fish_data %>% select(barcode, row) %>% distinct()
sites_p26ilror <- left_join(sites_p26ilror, barcodes_tojoin, by = "row")
# This will allow for a join with sites_aquaareas5m2. 

head(barcodes_tojoin)
```
