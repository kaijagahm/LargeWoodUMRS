...
begyear <- 1992.5 # where does this come from?
y0 <- makemod(lm1, begyear)
y1 <- makemod(lm1, changeyear)
summary(lm1)
mn <- p4[1:changepoint,] %>% pull(propwood) %>% mean()

segments(begyear, mn, changeyear, mn, lwd = 3, lty = 2, col = "darkgrey")
...
baseplot_p4 + geom_vline(aes(xintercept = changeyear), 
                         col = "red",
                         lty = 2) + 
  geom_segment(aes(x = begyear, y = mn, xend = changeyear, yend = mn),
               lty = 2,
               col = "darkgrey")+
