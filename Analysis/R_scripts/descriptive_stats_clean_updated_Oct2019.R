...
arc <- rbind(all_reduced_clean, all2_reduced_clean) 

# convert `pool` to factor and relevel
arc$pool <- factor(arc$pool, levels(arc$pool)[c(2, 3, 1, 4, 5, 6)]) 
locate.nas(arc) # NA's have already been removed. The column `near_forest_class` has also been removed. 

#exclude BWC-O and MCB-W ************************add step to exclude psuedo shoreline here?*************************
