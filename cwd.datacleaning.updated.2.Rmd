---
title: "cwd.datacleaning.updated.2"
author: "Kaija Gahm"
date: "4/29/2020"
...
## Load data
```{r}
load("Analysis/data/outputs/fish_data_EF.Rda") 
  # Fish sampling data
fish_data_EF <- fish_data_EF %>% mutate(fishdata_rownum = 1:nrow(fish_data_EF)) #number the rows

sites_p4p8p13 <- read.csv("Analysis/data/inputs/DataSets_7_7/AttributeTables/sites_p4p8p13.txt") 
  # reprojected data

rivmi <- read.table("Analysis/data/inputs/p4p8p13_rivmile.txt", sep = ",", header = T) 
  # river mile data

sites_aa_5m <- read.csv("Analysis/data/inputs/DataSets_7_7/AttributeTables/sites_aquaareas5m.txt") 
...
## Save data
```{r}
save(sites_ur, file = "Analysis/data/outputs/sites_ur.Rda")
```
...
sites_lr <- sites_lr %>% dplyr::select(-toexclude) # drop columns
names(sites_lr)
```
...
## Aggregate strata
We want to end up with BWC, IMP, MCB, and SCB.

### Upper
```{r}
sites_ur <- sites_ur %>% 
  dplyr::mutate(stratum_collapsed = 
                  case_when(stratum %in% c("BWC-O", "BWC-S") ~ "BWC",
                            stratum %in% c("IMP-O", "IMP-S") ~ "IMP",
                            stratum %in% c("MCB-U") ~ "MCB",
                            stratum  == "SCB" ~ "SCB"))
```

### Lower
```{r}
table(sites_lr$stratum)
